...
export default function App() {
  ...
  const saveDbToCache = async () => {
    try {
      // Garantir que o diretório de destino existe
      await FileSystem.makeDirectoryAsync(FileSystem.cacheDirectory + 'SQLite/', { intermediates: true });

      // Ler o banco de dados atual em Base64
      const base64 = await FileSystem.readAsStringAsync(
        FileSystem.documentDirectory + 'SQLite/example.db',
        {
          encoding: FileSystem.EncodingType.Base64
      ...
      await FileSystem.writeAsStringAsync(
        FileSystem.cacheDirectory + 'SQLite/example.db',
        base64,
        { 
          encoding: FileSystem.EncodingType.Base64 
        }
      ...
      console.log("Banco de dados salvo no cache com sucesso!");
      alert("Salvo no Cache!");
    } catch (error) {
      console.error("Erro ao salvar no cache:", error);
      alert("Erro ao salvar no cache: " + error.message);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Salvar DB no Cache" onPress={saveDbToCache} />
